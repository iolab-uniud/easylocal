...
namespace EasyLocal
{
  ...
  namespace Core
  {
    ...
    protected:
      ...
      void CompleteIteration(const Input& in);
      // parameters
    ...
    // FIXME
    template <class Input, class State, class Move, class CostStructure>
    void AbstractSimulatedAnnealing<Input, State, Move, CostStructure>::InitializeRun(const Input& in)
    {
     ...
     */
    template <class Input, class State, class Move, class CostStructure>
    void AbstractSimulatedAnnealing<Input, State, Move, CostStructure>::SelectMove(const Input& in)
    {
      // TODO: it should become a parameter, the number of neighbors drawn at each iteration (possibly evaluated in parallel)
      size_t sampled;
      double t = this->temperature;
      EvaluatedMove<Move, CostStructure> em = this->ne.RandomFirst(in, *this->p_current_state, this->max_neighbors_sampled - neighbors_sampled, sampled, [t](const Move &mv, const CostStructure &move_cost) {
        double r = std::max(Random::Rand(0.0, 1.0), 1E-16);
     ...
     */
    template <class Input, class State, class Move, class CostStructure>
    void AbstractSimulatedAnnealing<Input, State, Move, CostStructure>::CompleteMove(const Input& in)
    {
      neighbors_accepted++;
    }
     ...
     */
    template <class Input, class State, class Move, class CostStructure>
    void AbstractSimulatedAnnealing<Input, State, Move, CostStructure>::CompleteIteration(const Input& in)
    {
