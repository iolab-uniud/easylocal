...
namespace EasyLocal {
  ...
  namespace Core {
    ...
    class SimulatedAnnealingWithReheating : public SimulatedAnnealingEvaluationBased<Input, State, Move, CFtype, CostStructure>
    {
    public:
    ...
    SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::SimulatedAnnealingWithReheating(const Input& in,
                                                                                                 StateManager<Input, State, CFtype, CostStructure>& sm,
                                                                                                 NeighborhoodExplorer<Input, State, Move, CFtype, CostStructure>& ne,
                                                                                                 std::string name)
    : SimulatedAnnealingEvaluationBased<Input, State, Move, CFtype, CostStructure>(in, sm, ne, name)
    {}
    
    template <class Input, class State, class Move, typename CFtype, class CostStructure>
    void SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::RegisterParameters()
    {
    ...
    template <class Input, class State, class Move, typename CFtype, class CostStructure>
    void SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::InitializeRun() throw (ParameterNotSet, IncorrectParameterValue)
    {
      SimulatedAnnealingEvaluationBased<Input, State, Move, CFtype, CostStructure>::InitializeRun();
      reheats = 0;
     ...
     */
    template <class Input, class State, class Move, typename CFtype, class CostStructure>
    void SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::CompleteMove()
    {
      SimulatedAnnealingEvaluationBased<Input, State, Move, CFtype, CostStructure>::CompleteMove();
    ...
    template <class Input, class State, class Move, typename CFtype, class CostStructure>
    bool SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::ReheatCondition()
    {
      if (max_reheats == 0)
        return false; //true;
     ...
     */
    template <class Input, class State, class Move, typename CFtype, class CostStructure>
    bool SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::StopCriterion()
    {
     ...
     */
    template <class Input, class State, class Move, typename CFtype, class CostStructure>
    std::string SimulatedAnnealingWithReheating<Input, State, Move, CFtype, CostStructure>::StatusString() const
    {
