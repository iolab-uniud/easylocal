...
namespace EasyLocal {
  namespace Core {       
    ...
    public:      
      virtual std::pair<Kick<State, MoveType, CostStructureType>, CostStructureType> SelectFirst(size_t length, const State &st) const throw (EmptyNeighborhood)
      {
        ...
        tbb::parallel_for_each(this->begin(length, st), this->end(length, st), [this, &st, &mx_first_kick, &first_kick, &first_kick_cost, &first_kick_found](Kick<State, MoveType, CostStructureType>& k) {
        ...
        if (!first_kick_found)
          return std::make_pair(Kick<State, MoveType, CostStructureType>::empty, CostStructureType(std::numeric_limits<CFtype>::infinity(), std::numeric_limits<CFtype>::infinity(), std::numeric_limits<CFtype>::infinity(), std::vector<CFtype>(CostComponent<Input, State, CFtype>::CostComponents(), std::numeric_limits<CFtype>::infinity())));
        return std::make_pair(first_kick, first_kick_cost);
      }

      
      virtual std::pair<Kick<State, MoveType, CostStructureType>, CostStructureType> SelectBest(size_t length, const State &st) const throw (EmptyNeighborhood)
      {
        ...
        tbb::parallel_for_each(this->begin(length, st), this->end(length, st), [this, &st, &mx_best_kick, &best_kick, &best_cost, &number_of_bests](Kick<State, MoveType, CostStructureType>& k) {
          CostStructureType cost(0, 0, 0, std::vector<CFtype>(CostComponent<Input, State, CFtype>::CostComponents(), 0));
          for (int i = 0; i < k.size(); i++)
          {
            if (!k[i].first.is_valid)
