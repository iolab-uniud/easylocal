...
namespace EasyLocal
{
  namespace Core
  {
    ...
    public:
      virtual std::pair<Kick<State, MoveType, CostStructureType>, CostStructureType> SelectFirst(size_t length, const Input& in, const State &st) const
      {
        ...
        tbb::parallel_for_each(this->begin(length, in, st), this->end(length, in, st), [this, &in, &mx_first_kick, &first_kick, &first_kick_cost, &first_kick_found](Kick<State, MoveType, CostStructureType> &k) {
      ...
      virtual std::pair<Kick<State, MoveType, CostStructureType>, CostStructureType> SelectBest(size_t length, const Input& in, const State &st) const
      {
        ...
        tbb::parallel_for_each(this->begin(length, in, st), this->end(length, in, st), [this, &in, &mx_best_kick, &best_kick, &best_cost, &number_of_bests](Kick<State, MoveType, CostStructureType> &k) {
          CostStructureType cost(0, 0, 0, std::vector<CFtype>(this->sm.CostComponents(), 0));
          for (int i = 0; i < k.size(); i++)
          {
            if (!k[i].first.is_valid)
      ...
      virtual std::pair<Kick<State, MoveType, CostStructureType>, CostStructureType> SelectRandom(size_t length, const Input& in, const State &st) const throw(EmptyNeighborhood)
      {
