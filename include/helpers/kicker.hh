...
template <class Solution, class Move, class CostStructure>
Kick<Solution, Move, CostStructure> Kick<Solution, Move, CostStructure>::empty;

template <class Solution, class Move, class CostStructure>
std::ostream &operator<<(std::ostream &os, const Kick<Solution, Move, CostStructure> &k)
{
  os << "{";
...
class FullKickerIterator : public std::iterator<std::input_iterator_tag, Kick<Solution, Move, CostStructure>>
{
  friend class Kicker<Input, Solution, Move, CostStructure>;

public:
  typedef typename Kicker<Input, Solution, Move, CostStructure>::MoveRelatedness MoveRelatedness;
  ...
  const Kick<Solution, Move, CostStructure> &operator*() const
  {
    return kick;
  }
  Kick<Solution, Move, CostStructure> &operator*()
  {
    return kick;
  }
  const Kick<Solution, Move, CostStructure> *operator->() const
  ...
  bool operator==(const FullKickerIterator<Input, Solution, Move, CostStructure> &it2) const
  {
    if (end && it2.end)
      return true;
    return (end == it2.end && length == it2.length && kick_count == it2.kick_count && &start_state == &it2.start_state);
  }
  bool operator!=(const FullKickerIterator<Input, Solution, Move, CostStructure> &it2)
  {
    if (end && it2.end)
      return false;
    return (end != it2.end || length != it2.length || kick_count != it2.kick_count || &start_state != &it2.start_state);
...
protected:
  FullKickerIterator(size_t length, const NeighborhoodExplorer<Input, Solution, Move, CostStructure> &ne, const Solution &state, const MoveRelatedness &RelatedMoves, bool end = false)
...
template <class Input, class Solution, class Move, class CostStructure>
class SampleKickerIterator : public std::iterator<std::input_iterator_tag, Kick<Solution, Move, CostStructure>>
{
  friend class Kicker<Input, Solution, Move, CostStructure>;
...
public:
  ...
  bool operator==(const SampleKickerIterator<Input, Solution, Move, CostStructure> &it2) const
  {
    if (end && it2.end)
      return true;
    return (end == it2.end && length == it2.length && kick_count == it2.kick_count && &start_state == &it2.start_state);
  }
  bool operator!=(const SampleKickerIterator<Input, Solution, Move, CostStructure> &it2)
  {
    if (end && it2.end)
      return false;
    return (end != it2.end || length != it2.length || kick_count != it2.kick_count || &start_state != &it2.start_state);
...
protected:
  ...
  SampleKickerIterator(size_t length, const NeighborhoodExplorer<Input, Solution, Move, CostStructure> &ne, const Solution &state, size_t samples, const MoveRelatedness &RelatedMoves, bool end = false)
...
public:
  ...
  /** Constructor.
       ...
       */
  Kicker(SolutionManager<Input, Solution, CostStructure>& sm, NeighborhoodExplorer<Input, Solution, Move, CostStructure> &ne, const MoveRelatedness &RelatedMoves = AllMovesRelated) : sm(sm), ne(ne), RelatedMoves(RelatedMoves) {}
  ...
  /** Generates the first improving kick.
       ...
       */
  virtual std::pair<Kick<Solution, Move, CostStructure>, CostStructure> SelectFirst(size_t length, const Solution &st) const
  {
    for (FullKickerIterator<Input, Solution, Move, CostStructure> it = begin(length, st); it != end(length, st); ++it)
    {
    ...
    return std::make_pair(Kick<Solution, Move, CostStructure>::empty, CostStructure(std::numeric_limits<CFtype>::infinity(), std::numeric_limits<CFtype>::infinity(), std::numeric_limits<CFtype>::infinity(), std::vector<CFtype>(sm.CostComponents(), std::numeric_limits<CFtype>::infinity())));
  }
  ...
  /** Generates the best kick.
       ...
       @return the cost of applying the kick to the @ref State
       */
  virtual std::pair<Kick<Solution, Move, CostStructure>, CostStructure> SelectBest(size_t length, const Solution &st) const
  {
    Kick<Solution, Move, CostStructure> best_kick;
  ...
  virtual std::pair<Kick<Solution, Move, CostStructure>, CostStructure> SelectRandom(size_t length, const Solution &st) const
  {
    SampleKickerIterator<Input, Solution, Move, CostStructure> random_it = sample_begin(length, st, 1);
    CostStructure cost(0, 0, 0, std::vector<CFtype>(sm.CostComponents(), 0));
  ...
  /** Executes a kick on a state.
       ...
       */
  virtual void MakeKick(Solution&st, const Kick<Solution, Move, CostStructure> &kick) const
  {
    st = kick[kick.size() - 1].second;
  }

  FullKickerIterator<Input, Solution, Move, CostStructure> begin(size_t length, const Solution &st) const
  {
    return FullKickerIterator<Input, Solution, Move, CostStructure>(length, ne, st, RelatedMoves);
  }

  FullKickerIterator<Input, Solution, Move, CostStructure> end(size_t length, const Solution &st) const
  {
    return FullKickerIterator<Input, Solution, Move, CostStructure>(length, ne, st, RelatedMoves, true);
  }

  SampleKickerIterator<Input, Solution, Move, CostStructure> sample_begin(size_t length, const Solution &st, size_t samples) const
  {
    return SampleKickerIterator<Input, Solution, Move, CostStructure>(length, ne, st, samples, RelatedMoves);
  }

  SampleKickerIterator<Input, Solution, Move, CostStructure> sample_end(size_t length, const Solution &st, size_t samples) const
  {
    return SampleKickerIterator<Input, Solution, Move, CostStructure>(length, ne, st, samples, RelatedMoves, true);
  }
