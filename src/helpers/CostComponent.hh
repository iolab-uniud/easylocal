...
/** 
  @brief The class CostComponent manages one single component of the cost, 
  either hard or soft.
  The responsibility of this class is to compute a component of cost based
  on the information contained in a state. It doesn't handle delta costs, as
  they are treated in @ref DeltaCostComponent.
...
public:
  ...
  /** Print the details of the cost component.
      @param os the output stream where the description has to be printed
  */
  void Print(std::ostream& os = std::cout) const;

  /** Compute this component of cost with respect to a given state and 
  ...
  */
  ...
  /** Compute this component of cost with respect to a given state.
      ...
      @remarks internally calls @ref ComputeCost and multiplies the result for
      the weight of the cost component.
  */
  CFtype Cost(const State& st) const { return weight * ComputeCost(st); }

  /** Print the violations relative to this cost component with respect to 
      the specified state.
  ...
  */
  ...
  /** Tell if this cost component is soft
      @return rue if this cost component is soft, false otherwise
  */
...
CostComponent<Input,State,CFtype>::CostComponent(const Input& i, const CFtype& w, 
  bool hard, std::string e_name)
...
{ 
  os  << "Cost Component " << name << ": weight " << weight 
      << (is_hard ? "*" : "") << std::endl; 
}
